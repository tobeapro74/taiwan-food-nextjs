...
### 1.2 검색바
- 실시간 자동완성 (1글자 이상, 정적 데이터)
- 최대 8개 검색 제안
- 전체 검색 시 정적 데이터 + 사용자 등록 맛집(MongoDB) 통합 검색
- 맛집명, 위치, 야시장, 카테고리, 특징 검색 지원
- **음식 종류 키워드 검색**: 음식명으로 검색하면 관련 카테고리 전체 결과 표시
  - 예: "누가크래커" → 디저트 카테고리 전체, "라떼" → 카페 카테고리 전체, "훠궈" → 탕류 카테고리 전체
  - 키워드 매핑: 디저트(빙수, 케이크, 마카롱 등), 카페(라떼, 밀크티, 버블티 등), 면류(우육면, 라멘 등), 밥류(루러우판, 지파이 등), 탕류(훠궈, 마라 등), 만두(샤오롱바오, 딤섬 등), 길거리음식(꼬치, 튀김 등)
...
### 5.4 AI 리뷰 분석 섹션 (2026.05.20 추가)
- Google 리뷰 캐시 완료 후 자동 실행 (순서 보장)
- Claude Haiku(`claude-haiku-4-5-20251001`)로 리뷰 텍스트 분석
- **대표 메뉴**: 리뷰에 언급된 음식/메뉴명 최대 3개 (주황색 태그, 원어 유지)
- **대표 리뷰**: 가장 유익한 리뷰 핵심 요약 최대 3개 (파란 카드)
- 중국어·일본어·영어 등 다국어 리뷰 → **한국어로 번역** 요약
...
### 9.3 성능 최적화 (2026.05.20 업데이트)
- Next.js App Router
- 정적 데이터 캐싱
- 이미지 lazy loading
- **SWR**: 홈 데이터(`/api/home-data`) 5분 dedup 캐시, 포커스 재검증 비활성
- **place_photos_cache 우선 조회**: 홈 이미지 Cloudinary URL 즉시 반환
- **MongoDB 연결 풀**: minPoolSize=2, retryWrites, socketTimeout 설정
- **번들 최적화**: recharts, sonner tree-shaking 추가
- **Vercel 한국 리전(icn1)**: 한국 유저 응답속도 단축
...
### 10.1 위치 설정
- GPS 기반 현재 위치 자동 탐지
- **GPS 실패 시 시먼딩 기본 위치로 자동 폴백** (25.0421, 121.5081)
  - 위치 권한 거부, GPS 타임아웃 등 모든 실패 상황에서 동작
  - App Store 심사 환경(미국 VPN/시뮬레이터) 대응
- 주소 검색으로 위치 설정
- 수동 좌표 입력
...
### 15.4 z-index 체계
- 바텀 시트 (Sheet Overlay/Content): `z-[95]`
- 하단 네비게이션: `z-[90]`
- 기본 모달: `z-[100]`
- 확인 모달 (중첩): `z-[110]`
- 회원탈퇴 모달: `z-[200]`
...
### 16.1 일정 생성
- 여행 일수 (1~14일)
- 인원 구성 (연령대별/성별 — 10대~60대+, 남/여 각각 설정)
- 취향 선택 (맛집, 카페, 쇼핑, 문화, 야경, 자연)
- 여행 목적 복수 선택 (맛집투어, SNS감성, 힐링, 쇼핑, 문화체험)
- 입국/출국 시간대 선택
- 숙소 지역 선택
- **일정 작성 중 목록 이동 시 확인 다이얼로그** 표시

### 16.2 일정 결과 화면
- 일차별 카드 (접기/펼치기)
- 시간대별 활동 표시 (오전/점심/오후/저녁/밤)
- 장소별 평점, 위치, 추천 이유, 꿀팁 표시
- **이동 정보 표시** (이전 장소→현재 장소 이동수단, 소요시간, 대화체 안내)
- **AI 대화체 추천 이유**: 여행자 구성(연령/성별)에 맞춘 친근한 대화체 멘트
- 여행 팁 및 예상 예산 안내
- **저장 전 이탈 시 확인 다이얼로그** (뒤로가기/목록 이동 시)
...
- **사진 그리드**: 2열 그리드 레이아웃
- **최대 10장** 사진 표시
- **사진 캐싱 구조** (2026.05.20 개선):
  - 전체 81개 정적 맛집 사진 10장씩 Cloudinary에 미리 업로드
  - MongoDB `place_photos_cache` 컬렉션에 영구 URL 저장
  - 모달 열 때 캐시에서 즉시 반환 (추가 API 호출 없음)
  - 캐시 없는 맛집은 `place-photo` API로 fallback (1장)
  - 워밍업 API: `/api/cache/warm-photos?key=admin123&offset=0&limit=5`
...
- 로그인 사용자만 저장 가능
- 저장된 일정 목록 화면
  - 일정 제목, 여행 일수, 숙소 지역, 저장일 표시
  - **연령대별 남녀 구성 요약 표시** (예: `40대여(2)+60대여(1)`)
- 일정 상세 보기 (저장된 일정은 "일정 저장하기" 버튼 숨김)
- 일정 삭제 기능 (ConfirmDialog로 삭제 확인)

### 16.6 AI 프롬프트 동적 생성
- **여행자 구성 분석**: 연령대별/성별 인원을 분석하여 프롬프트를 동적 생성
  - 단일 연령대 vs 다양한 연령대 자동 판별
  - 성별 분석 (여성/남성/혼성)
  - 시니어/중장년/청년 여부 감지
- **조건부 프롬프트 블록**:
  - 다양한 연령대 → "다양한 연령대 배려 원칙" 포함
  - 단일 연령대 → "여행자 맞춤 원칙" (선택한 취향/목적에 집중)
  - 시니어 포함 → 이동 거리/휴식 시간 배려
  - 성별 특화 → 여성(감성카페, 포토스팟), 남성(맛집, 야시장)
- **대화체 가이드 스타일**: reason 필드에 여행자에게 직접 말하는 친근한 멘트
- **이동 정보 (travelFromPrev)**: 각 활동 간 이동수단, 소요시간, 대화체 안내
...
- 헤더 우측 테마 토글 버튼
- `localStorage` 기반 테마 저장 (새로고침 시 유지)
- `ThemeProvider` 컴포넌트로 전역 테마 관리
- CSS 변수 기반 다크 테마 (`--background`, `--foreground`, `--card` 등)

### 20.2 다크 테마 색상
- 배경: `hsl(15, 15%, 7%)` (Deep Warm Dark)
- 카드: `hsl(15, 10%, 11%)`
- 보더: `hsl(15, 6%, 20%)`
- 텍스트: `hsl(15, 5%, 92%)`
- 서브텍스트: `hsl(15, 4%, 55%)`

## 21. Pull-to-Refresh (당겨서 새로고침)

### 21.1 기능
- 홈 화면에서 아래로 당기면 새로고침
- 스피너 애니메이션 표시
- 새로고침 완료 후 데이터 갱신
- `usePullToRefresh` 커스텀 훅으로 구현

### 21.2 동작 조건
- `scrollY === 0`일 때만 활성화
- 수직 방향 50px 이상 당기면 새로고침 트리거
- 수평 스크롤/스와이프백과 충돌 방지
...
### 22.1 홈 화면 퀵 액세스
- 2열 그리드 레이아웃
- 여행 가이드 📖, 화장실 찾기 🚽 타일 (상단 2열)
- AI 추천 🤖 타일 (col-span-2, primary→accent 그라데이션)
- 각 타일 클릭 시 해당 화면으로 이동
- 다크모드 호환

## 23. 시간대별 맛집 추천

### 23.1 시간대 구분 (대만 시간 UTC+8 기준)
| 시간대 | 시간 | 이모지 | 설명 |
|--------|------|--------|------|
| 아침 | 06:00~11:00 | 🌅 | 든든한 아침 맛집 (또우장, 퐌투안, 브런치) |
| 점심 | 11:00~14:00 | 🍽️ | 인기 맛집 추천 (면류, 밥류, 만두) |
| 오후 | 14:00~17:00 | ☕ | 디저트 & 카페 |
| 저녁 | 17:00~21:00 | 🌙 | 저녁 맛집 (훠궈, 마라, 밥류) |
| 야식 | 21:00~06:00 | 🌃 | 야시장 맛집, 길거리음식 |

### 23.2 표시 방식
- 홈 화면 검색바 아래에 시간대별 인사말 + 추천 맛집
- 밝은 카드 스타일 (bg-card, border)
- 수평 스크롤 카드 레이아웃
- 카테고리 + 키워드 기반 휴리스틱 매핑
...
### 24.1 Web API 기반
- `navigator.vibrate()` 사용 (Android Chrome 지원)
- iOS Safari/WKWebView에서는 안전한 no-op fallback

### 24.2 iOS 네이티브 브리지
- `WKScriptMessageHandler`로 네이티브 햅틱 연동
- `UIImpactFeedbackGenerator` / `UISelectionFeedbackGenerator` 사용
- Web에서 `window.webkit?.messageHandlers?.haptic?.postMessage()` 호출

### 24.3 적용 위치
- 하단 네비게이션 탭 클릭 (selection)
- 맛집 카드 탭 (impact)
- 카테고리 시트 옵션 선택 (selection)
- PTR 새로고침 완료 (notification)
- 벤토 그리드 버튼 탭 (impact)

## 25. 카드 롱프레스 미리보기 (Peek Preview)

### 25.1 제스처
- 500ms 롱프레스로 미리보기 활성화
- 이동 10px 이상 시 타이머 취소 (스크롤/PTR과 충돌 방지)
- `-webkit-touch-callout: none`으로 iOS 기본 컨텍스트 메뉴 방지

### 25.2 미리보기 UI
- 풀스크린 블러 오버레이 (z-[95])
- 중앙 카드: 이미지 + 맛집명 + 평점 + 위치 + 특징
- Glass morphism 스타일 (backdrop-blur)
- 스프링 스케일 애니메이션 (0.9 → 1.0)
- "상세보기" 버튼으로 상세 화면 이동

## 26. AI 맛집 추천

### 26.1 프리셋 추천
- 8개 프리셋 칩: 매운음식 🌶️, 가성비 👍, 데이트 💕, 혼밥 🍜, 야시장 🌃, 디저트 🍧, 현지로컬 🏠, 면요리 🍝
- 칩 클릭 시 MongoDB 캐시에서 즉시 반환 (OpenAI 호출 없음)
- 캐시 미스 시에만 OpenAI 호출 후 결과 캐싱
- `/api/ai-recommend/seed` 엔드포인트로 프리셋 캐시 사전 생성

### 26.2 자유 텍스트 입력
- 입력창 + 전송 버튼
- 사용자 맞춤 질문으로 AI 추천 요청
- 검색 중 풀스크린 스핀 모달 표시 (메시지 순환 애니메이션)

### 26.3 AI 추천 결과
- GPT-4o-mini 기반 맛집 추천
- 추천 이유 + 매칭 점수 표시
- 실제 DB 맛집과 매칭하여 hallucination 방지
- 현재 시간대 컨텍스트 자동 포함
- AI 추천 팁 배너 표시

### 26.4 Bento Grid 연동
- 홈 화면 벤토 그리드에 "AI 추천 🤖" 타일
- 전체 너비 (col-span-2) primary→accent 그라데이션

## 27. 온보딩 캐러셀

### 27.1 표시 조건
- 첫 방문 시 1회만 표시
- `localStorage` `onboarding_completed` 키로 관리
- 풀스크린 오버레이 (z-[100])

### 27.2 스텝 구성
1. 🍜 환영 — "대만맛집에 오신 것을 환영합니다"
2. 🗺️ 핵심 기능 — 맛집 탐색, 야시장, 근처 맛집
3. 📅 AI 일정 — AI 여행 일정 생성 기능 소개
4. 🚀 시작하기 — CTA 버튼

### 27.3 네비게이션
- 하단 "다음" / "시작하기" 버튼
- 우상단 "건너뛰기" 버튼
- 좌우 스와이프로 스텝 이동 (50px 임계값, 수평 전용)
- 하단 도트 인디케이터
